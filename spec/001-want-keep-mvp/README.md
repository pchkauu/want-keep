...
Пакет фиксирует согласованный MVP и полный backlog для последующего выполнения AI-агентом. Приложение ещё не реализовано. Все 15 исходных возможностей и все шесть интеграций обязательны; работа над частью функций не означает готовность полного MVP.
...
Начать с task-0.1–task-0.9 и записать evidence исследования. При отсутствии безопасно предоставленного доступа нельзя заявлять live-проверку. task-0.10 закрывает контракты, обновляет RU/EN и проверяет SDD Ready. До этого application implementation заблокирована; `plan.md` намеренно отсутствует. Далее выполнять зависимости и критерии конкретной карточки, а не считать её название достаточным заданием.
