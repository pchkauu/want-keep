...
Task-7.1 реализует `/login`, `/setup`, `/invite`, `/recovery`, `/onboarding` и минимальный `/settings/security` для перевыпуска личных recovery-кодов после свежего passkey. Выдача/перевыпуск/отзыв приглашения находятся в onboarding; принятие — на `/invite` после server preview. Fragment с токеном удаляется до рендера. Создание семьи и принятие приглашения ведут в onboarding, обычный вход — в обзор. Наличный счёт сохраняется через task-2.1; подключение банка и приглашение партнёра не обязательны для продолжения.

Task-7.9 добавляет `/settings/household` и постоянный переключатель «Семья / участник» в верхней части защищённой оболочки. Имя вошедшего пользователя показывается отдельно. Разрез кодируется только как `?view=household` или `?view=member&member=<userId>`; неизвестный, отсутствующий или неактивный участник канонизируется в семейный вид. Переход между разделами переносит только `view/member`, не чужие фильтры. Семейный вид показывает все счета по принадлежности, индивидуальный — общие счета и личные счета выбранного участника. Это фильтр отображения: личный наличный счёт всегда создаётся за actor из сессии, а скрытые строки не исключаются из семейного денежного пула. Подробности и доказательства: [task-7.9](evidence/task-7.9-household-context.md).

Язык выбирается в порядке сохранённого выбора браузера, подтверждённого профиля, поддерживаемого языка браузера, RU. Только язык входит в localStorage. Переключение сохраняет форму и церемонию; серверная запись предпочтений остаётся task-7.14. Незавершённые финансовые разделы показывают честное состояние недоступности. Подробности и границы приёмки: [task-7.1](evidence/task-7.1-access.md).

URL отчётов/списков хранит несекретные period/month, view/member, currency, account/category, query и сортировку. Не помещать в URL чеки, секреты, сообщения или финансовый черновик. Курсор/позиция списка восстанавливаются из состояния навигации при возврате; если данные изменились, показать обновлённый список и сохранить фильтры. Выбор участника меняет только представление, не principal, actor, owner или доступ.
