...
Миграция 005 расширяет схему; 001–004 не меняются. OpenAPI/generated Go/TypeScript обновляются вместе, развёрнутых клиентов нет. Task-3.3 и адаптеры подключают owner grant, secret purpose, admission/lease и quarantine к реальному IO; task-5.3 — разбор чека; task-7.1 — UI; task-8.1/8.2 — размещение, расписание очистки, ротация и резервирование keyring вместе с ciphertext. [Проверки и ограничения](evidence/task-1.5-privacy.md).
