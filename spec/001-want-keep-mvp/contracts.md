...
`GET/POST /transactions`, `GET /transactions/{transactionId}` и `POST /transfers` работают через общий cookie/Origin/CSRF/no-store guard. Ручные записи доступны обоим участникам на любом счёте семьи. Actor определяется доверенной сессией; payer, владелец и автор независимы. Вложение должно принадлежать семье, быть принято и относиться к выбранному счёту. Merchant/note сохраняются. Пока доступны только unresolved allocation и новые переводы; category/shares и непустой existingTransactions отвергаются явно (422 feature_unavailable), без игнорирования полей. Остальные продуктовые маршруты не активируются.
