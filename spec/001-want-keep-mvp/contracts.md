...
Funding USDT/USDC/ETH/BTC, используемый Easy Earn и P2P обязательны. Остальные продукты не блокируют; официальный API приоритетен. [Матрица read-маршрутов и ограничений](evidence/bybit.md), [синтетические проекции](evidence/bybit.samples.json) задают вход task-0.10, не готовую provider schema. Read POST `/v5/p2p/order/simplifyList` и `/v5/p2p/order/info` разрешаются отдельно от финансовых write POST; нужны readOnly и фактическая eligibility аккаунта. Нет ключа и подтверждённого P2P API-доступа — partial/blocked, не пустой успешный импорт. Секрет `apiKey`, возвращаемый query-api, исключается до логов/AI/evidence.

Identity разделяет household/provider/site/UID, accountType FUND/asset и record ID конкретного журнала. Ротация ключа не создаёт счёт; разные UID участников не объединяются. Funding `currcCursor` и IDs деталей — исходные записи, не автоматически разные финансовые события. Связь/статус/gross/net подтверждаются структурированным контрактом; переводные business labels не определяют критическую классификацию. Withdrawal fee, две Convert legs, Earn accrual/payout/Funding и P2P/bank не удваивают эффект. Principal не является доходом; неизвестная связь вызывает уточнение.

USDC отличается от USD/USDT; похожие USDC.E/USDCX не объединяются. Native decimal не ограничивается округлением UI или subscription precision; missing/bonus/hold не становятся доступным нулём. Funding time filters в секундах, многие детали в ms; отдельные envelopes/units устанавливаются на границах. Yield list/yield, internal-deposit timestamp и документированный APR с %/часовыми точками требуют явной проверки. Проверять used Flexible/Fixed, не принимать пустой список за отсутствие истории.

Durable checkpoints, перекрытие окон, версии и границы retention обязательны; API yield описывает три месяца, P2P — максимум 180 дней, Convert web — с 2025-09-10. Запрос до доступной истории даёт partial и начальный остаток/доказательство покрытия, не вымышленную полноту. BYBIT-B02–B05 закрывает task-0.10 до task-4.4; browser fallback допустим только по доказанному разрешённому структурированному read-контракту. Это целевое уточнение версии 6 без изменений финансового runtime/БД.
