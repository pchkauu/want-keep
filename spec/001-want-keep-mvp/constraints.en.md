...
The contract fails closed: read capabilities only, strict JSON, aligned Unicode/byte limits and an exact job/binding/revision/cursor echo. The exact admitted build's manifest restricts product, log namespace, record kind and required read action before evidence storage. A typed provider outcome preserves retry/reauthentication/terminal lifecycle under the same fence. A stale result crosses only the evidence/quarantine boundary. Downstream tasks prove live provider routes, network isolation and permission; a synthetic gateway does not admit production deployment.
