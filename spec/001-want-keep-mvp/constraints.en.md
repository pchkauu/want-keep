...
`integrations/application` coordinates `ProviderGateway`, `EvidenceStore`, the accounts importer, ledger source writer and `connections/admission`. The gateway is created from server-owned configuration with the full immutable D-43 binding; the application compares it with the job binding before manifest/provider IO. Provider IO runs only after the pre-read fence and outside the financial transaction. Raw evidence is durable before `CommitPage`; the callback performs no external IO. An account may be resolved without a balance observation; every balance/posting reference requires an account descriptor in the same self-contained page. Server principals, external owners, internal IDs, revisions and timestamps never come from the payload.

The contract fails closed: read capabilities only, strict JSON, canonical UTC instants with `Z`, aligned Unicode code-point/byte limits and an exact job/binding/revision/cursor echo. NUL and lone surrogates are rejected before identity/storage; composite keys use structural encoding. Gaps are unique and bounded, while an omitted lookback never collapses into explicit zero. The exact admitted build's manifest is bound to the provider and restricts product, log namespace, record kind and required read action before evidence storage. A card alias permits only a safe description and its own last four ASCII digits. Separate fee postings are supported, but free-form `feeId` cannot cross the v10 boundary without typed correspondence. Server-side account/source ambiguity retains evidence, makes coverage partial and does not roll back the page's supported records. A typed provider outcome atomically associates evidence with its household/job and preserves retry/reauthentication/terminal lifecycle under the same fence. A stale result crosses only the evidence/quarantine boundary. Downstream tasks prove live provider routes, network isolation and permission; a synthetic gateway does not admit production deployment.
