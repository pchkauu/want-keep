...
The contract fails closed: read capabilities only, strict JSON, canonical UTC instants with `Z`, aligned Unicode code-point/byte limits and an exact job/binding/revision/cursor echo for both pages and provider failures. The current cursor participates in the attempt fence, so a delayed outcome from an earlier page cannot change job state. NUL and lone surrogates are rejected before identity/storage; composite keys use structural encoding. Gaps are unique and bounded, while an omitted lookback never collapses into explicit zero. The exact admitted build's manifest is bound to the provider and restricts product, log namespace, record kind and required read action before evidence storage. Confirmed provider mappings, including Raiffeisen/Ozon `RUR → RUB`, live in the accounts-owned policy and preserve the raw code; unknown symbols are never mapped heuristically. A card alias permits only a safe description and its own last four ASCII digits. Separate fee postings are supported, but free-form `feeId` cannot cross the v10 boundary without typed correspondence. Server-side account/source ambiguity retains evidence, makes coverage partial and does not roll back the page's supported records. A typed provider outcome atomically associates evidence with its household/job and preserves retry/reauthentication/terminal lifecycle under the same fence. A stale or rejected result crosses only the evidence/quarantine boundary. Downstream tasks prove live provider routes, network isolation and permission; a synthetic gateway does not admit production deployment.
