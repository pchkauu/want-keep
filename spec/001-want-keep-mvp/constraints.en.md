...
The contract fails closed: read capabilities only, strict JSON, canonical UTC instants with `Z`, aligned Unicode code-point/byte limits and an exact job/binding/revision/cursor echo. NUL and lone surrogates are rejected before identity/storage; composite keys use structural encoding. The exact admitted build's manifest is bound to the provider and restricts product, log namespace, record kind and required read action before evidence storage. A card alias permits only a safe description and its own last four ASCII digits. A typed provider outcome preserves retry/reauthentication/terminal lifecycle under the same fence. A stale result crosses only the evidence/quarantine boundary. Downstream tasks prove live provider routes, network isolation and permission; a synthetic gateway does not admit production deployment.
