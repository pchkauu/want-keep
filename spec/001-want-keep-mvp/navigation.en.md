...
Task-7.9 adds `/settings/household` and a persistent Household/member selector to the protected shell header. The signed-in member remains visible separately. The view is encoded only as `?view=household` or `?view=member&member=<userId>`; an unknown, missing or inactive member is canonicalized to household view. Section navigation carries only `view/member`, never unrelated filters. Household view groups every account by ownership; member view shows household accounts and that member's personal accounts. This is presentation only: a personal cash account is always created for the session actor, and hidden rows remain part of the household money pool. Details and evidence: [task-7.9](evidence/task-7.9-household-context.en.md).
...
Report/list URLs retain nonsecret period/month, view/member, currency, account/category, query and sort. Never put receipts, secrets, messages or financial drafts in URLs. Cursor/list position restores from navigation state on return; if data changed, show refreshed rows with preserved filters. Member selection changes presentation only, never principal, actor, owner or authority.
