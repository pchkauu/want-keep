...
Nested transactional callbacks use PostgreSQL savepoints: failures restore writes and scope state before refusal/quarantine handling. A rollback/release failure blocks the outer commit. Command registration requires `WithinNewHousehold` without an ambient transaction and rejects nested invocation before any write, keeping pending independently durable. PostgreSQL regressions cover nested rollback, restored lock metadata, nested registration refusal and a late-stale page inside an outer admission scope.
