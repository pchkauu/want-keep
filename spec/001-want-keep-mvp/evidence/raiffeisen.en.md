...
RAIF-B02/B03/B04/B06 above move from a global SDD blocker into the executable task-4.2 gate. The target contract permits CAMT entry to 1:N transaction details; use NtryRef/AcctSvcrRef/EndToEndId only when present and within a proven scope. Statement/report ID is retained only as provenance and is not part of transaction identity. Fallback is a transaction-scoped, documented-order fingerprint of non-empty structured reference/party/account/remittance/bank-code fields proven invariant across overlapping camt.052/camt.053; amount/time are excluded. An insufficient or collision-prone fingerprint yields `source_ambiguous` without posting. Corrections/reversals are retained as revisions.
