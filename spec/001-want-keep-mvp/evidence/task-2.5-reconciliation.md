...
Домен сверки хранит lifecycle, результат, replay и четыре независимых компонента. Application-сервис строит историческую проекцию, дедуплицирует replay, координирует admission и создаёт разрешённые adjustments через публичный контракт ledger. Storage сохраняет неизменяемые source observations и revisions миграции 011 после миграции 010 долговечных заданий. Delivery использует generated DTO, общий session/Origin/CSRF guard, подписанные курсоры и command recovery.
...
Проверяются новая observation и superseded lifecycle, повторная оценка после revision, исключения и undo, отсутствие события для идентичного результата, admission/reauth failure без фиктивного job, семейная изоляция и неизменность source evidence. Финансовая транзакция атомарно сохраняет pending replay и durable outbox-событие; worker после commit идемпотентно создаёт и присоединяет job только для точной revision. Replay сохраняет исходные binding/admission revision/connection generation. Completion выполняется внутри точной admitted `CommitPage`-транзакции, а терминальная ошибка провайдера — через эквивалентный admitted `CommitFailure`; household-only outcome отклоняется. Delayed source posting использует подтверждённый `postedAt`, reversal с неизвестным временем перехода остаётся incomplete, а многопроводочная операция применяется один раз. Resolution проверяет фактический ledger-эффект adjustment на все четыре компонента, включая связанное изменение available; результат `balanced` больше не сохраняет объяснение `balance_difference`, но сохраняет независимую stale-quality. HTTP-проверки охватывают фильтры, session-bound pagination, CSRF, безопасный 404, idempotent command recovery и изменённый payload. Миграционный тест обновляет базу 010 → 011, повторно применяет миграции и подтверждает неизменяемость истории под непривилегированной ролью.

После self review усилены четыре границы. Текущая revision сверки загружает только указанный ею replay request, поэтому завершённый запрос прежней revision не подменяет новый pending replay. Создание sync job и его присоединение к revision выполняются в одной admission → household транзакции: worker не видит job до фиксации ссылки. Неопределённый lifecycle делает unknown только затронутые компоненты, сохраняя доказанные значения остальных. При `version_conflict` разрешённая текущая revision сохраняется в результате команды и tombstone и возвращается тому же инициатору после восстановления результата.
