...
`make check` passed: formatting, lint, TypeScript, 118 web unit tests, Go/collector, build, SDD and reproducible OpenAPI. `make test-web FILTER=household` contains 11 tests, including a deterministic barrier between the new-session load and invitation issue. `make e2e SCENARIO=all` passed 28 scenarios: access, 19 component, 7 token and family-access. PostgreSQL household/accounts integration and both race suites passed against an isolated database. `git diff --check` passed.
