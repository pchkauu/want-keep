...
`make check` passed: formatting, lint, TypeScript, 120 web unit tests, Go/collector, build, SDD and reproducible OpenAPI. `make test-web FILTER=household` contains 13 tests, including deterministic barriers for invitation issue and revocation across a session change and degraded invitation API without losing household context. `make e2e SCENARIO=all` passed 28 scenarios: access, 19 component, 7 token and family-access. PostgreSQL household/accounts integration and both race suites passed against an isolated database. `git diff --check` passed.
