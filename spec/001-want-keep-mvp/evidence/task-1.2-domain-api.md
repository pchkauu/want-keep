...
D-43 реализован в connections/domain: default pending, отдельные provider/host checks, точное совпадение binding, блокировка failed/revoked, отклонение старого check и сброс evidence при rebind. Boundary публикует состояние; обратного command converter нет. Application admission service и атомарная проверка перед созданием job обязательны в task-3.3/task-4.x/task-8.1. Это domain/DTO coverage REQ-088/AC-106, не пройденный integration/security AC.
