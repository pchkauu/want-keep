...
Успешный ValuationObservation требует непустые source legs в обоих generated клиентах. Синтетический BTC/RUB round trip сохраняет observations CoinGecko и CBR, включая более раннюю effective date курса CBR и точные десятичные значения. Отсутствующие, пустые, null и неполные legs отклоняются для reference и executed observations; unavailable-оценка использует отдельный вариант ответа.
