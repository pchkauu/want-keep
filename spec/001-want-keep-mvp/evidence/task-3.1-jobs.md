...
- Решения о состоянии, причине и числе попыток принадлежат `Job.Complete`, `Job.Recover`, `Job.Cancel` и `Job.Reconcile`. Storage сохраняет эти решения под действующими блокировками и fences; recovery читает PostgreSQL-время и membership вместе с ограниченной пачкой заданий. Неначатое задание без handler переходит в waiting даже после длительного простоя worker. Уже потраченные попытки, отмена, отзыв membership и неизвестные внешние эффекты сохраняют свои ограничения. Исчерпание попыток при reconciliation сохраняет причину `attempts_exhausted`.
...
Отмена при отключении, изменении admission и устаревшем продолжении использует одно доменное решение. Типизированное reconciliation различает подтверждённый финальный эффект, промежуточную страницу и доказанное отсутствие эффекта; только отсутствие сохраняет существующее замещающее задание. Заблокированные записи и полученные решения читаются и сохраняются в одной транзакции.
