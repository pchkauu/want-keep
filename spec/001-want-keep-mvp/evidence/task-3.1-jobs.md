...
- Worker получает только свободное число jobs, восстанавливает principal из сохранённого actor и актуального membership, обновляет heartbeat и отменяет handler при потере lease. Handler обязан соблюдать context cancellation и не выполнять финансовые изменения во время `Prepare`.
- `Prepare` выполняет внешнюю работу; `Result.Apply` — только локальную DB-работу. Executor проверяет family/actor/job/attempt/token/target и lease до effect и при terminal commit. Effect, receipt и acknowledgement атомарны. Source handlers используют admission `CommitPage`; generic executor не предоставляет обход source fence.
