...
The suite checks RUB, USD, USDT, USDC, BTC and ETH at arbitrary precision; 50/50, 60/40, exact amounts, deterministic remainder and mixed receipt `1000 → A 400 + B 600`. It covers item overrides, a fully unresolved item split whose explicit reason survives correction/matching refresh, share-based and amount-based purchase-fallback recalculation without promoting derived item snapshots to explicit overrides, a third-asset fee and repeated `MembershipID` inputs across native assets. Rule precedence/conflict/preview, persisted `rule_conflict` for manual and imported facts, and rule archival after its merchant/category is archived are also covered. Confirmed merchant aliases without trusting provider IDs, ignored allocation from provider payloads, `not_applicable` imported income and rule allocation of only a transfer's expense fee are verified together with no retrospective change, correction/undo/source protection, the complete review hash and one effect across matching evidence.
