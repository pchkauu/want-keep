...
Вложенные транзакционные callback используют PostgreSQL savepoint: ошибка откатывает записи и состояние scope до обработки отказа/quarantine. Ошибка rollback/release блокирует внешний commit. Регистрация команды требует `WithinNewHousehold` без ambient transaction и отклоняет вложенный запуск до первой записи, чтобы pending был независимо зафиксирован. Новые PostgreSQL-сценарии проверяют nested rollback, восстановление lock metadata, отказ вложенной регистрации и late-stale page внутри внешнего admission.
