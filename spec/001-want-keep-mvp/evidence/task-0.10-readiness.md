...
| BLK-02 Raiffeisen | CAMT 1:N, scoped IDs, cross-report transaction fingerprint без amount/time, collision policy, revisions/reversals; statement ID — provenance; last confirmed CLBD с датой, остальные balances/fees unknown. | [Raiffeisen](raiffeisen.md), synthetic JSON/XML | task-4.2: OAuth lifecycle, full history, corrections, second account, conformance |
...
- Provider admission: server-owned exact binding environment/build/contract/allowlist/config/permission; task-4.x provider evidence + task-8.x host evidence; stale/missing binding → `provider_not_admitted` до collector IO.
...
| Один CAMT-факт без stable ID пришёл в 052 и 053 | Statement IDs — provenance; доказанный transaction fingerprint дедуплицирует между отчётами, иначе `source_ambiguous` без второй проводки |
...
| Admission относится к старому build/allowlist/config | `provider_not_admitted`; job/provider IO/source record/проводка не создаются до нового combined pass |
