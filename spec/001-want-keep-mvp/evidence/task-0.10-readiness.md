...
- Provider admission: aggregate/repository — application boundary `backend/internal/connections/admission/`, storage adapter — task-1.3; server-owned exact binding environment/build/contract/allowlist/config/permission и monotonic `admissionRevision`; atomic task-4.x provider evidence + task-8.x host evidence; stale/missing binding → `provider_not_admitted` до нового job/collector IO, commit-time stale result → quarantine без source/posting.
...
| Revoke/binding change происходит до enqueue или provider IO | Atomic invalidate/check+enqueue и collector recheck дают `provider_not_admitted`; job или provider IO не начинается |
| Revoke/binding change происходит после начала provider IO | Cancel best effort; result с прежним `admissionRevision` не проходит commit-time revalidation и остаётся в quarantine без source record/проводки |
