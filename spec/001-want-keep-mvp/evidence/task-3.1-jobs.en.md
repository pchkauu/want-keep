...
- State, reason and attempt decisions belong to `Job.Complete`, `Job.Recover`, `Job.Cancel` and `Job.Reconcile`. Storage persists these decisions under existing locks and fences; recovery reads PostgreSQL time and membership with a bounded job batch. Unstarted work without a handler enters waiting even after an extended worker outage. Consumed attempts, cancellation, revoked membership and unknown external effects retain their restrictions. Attempt exhaustion during reconciliation retains `attempts_exhausted` as its reason.
...
Cancellation on disconnect, admission invalidation and obsolete continuation uses one domain decision. Typed reconciliation distinguishes a confirmed final effect, a confirmed intermediate page and proven absence; only absence preserves an existing replacement. Locked rows and returned decisions are read and persisted in the same transaction.
