...
- State, reason and attempt decisions belong to `Job.Complete`, `Job.Recover` and `Job.Reconcile`. Storage persists these decisions under existing locks and fences; recovery reads PostgreSQL time and membership with a bounded job batch. Unstarted work without a handler enters waiting even after an extended worker outage. Consumed attempts, cancellation, revoked membership and unknown external effects retain their restrictions. Attempt exhaustion during reconciliation retains `attempts_exhausted` as its reason.
