...
D-43 lives in connections/domain: pending by default, separate provider/host checks, exact binding equality, failed/revoked blocking, stale-check rejection and evidence reset on rebind. The boundary exposes state without an inverse command converter. The application admission service and atomic enforcement before job creation remain mandatory in task-3.3/task-4.x/task-8.1. This covers REQ-088/AC-106 domain/DTO behavior, not the full integration/security AC.
