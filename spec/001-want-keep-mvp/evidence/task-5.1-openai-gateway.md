...
- После generation только подтверждённый отказ без списания (`429`) допускает одну новую оплачиваемую попытку; следующий такой отказ завершает job постоянной ошибкой. Input counting, pre-generation outage и восстановленный резерв без external-started marker не расходуют этот лимит. 408/5xx, таймаут либо неопределённый transport outcome после marker становятся `unknown`, сохраняют provider observation, резерв и job `unresolved`. Фактический input usage принимается только в диапазоне от подсчитанного значения до `count + 32`, output — от 1 до route cap. Отсутствующий/противоречивый usage или несовпадающая provider model также переводятся в `unknown`. Новый месяц, смена ключа или проекта не снимают barrier. Оператор подтверждает точную стоимость либо отсутствие списания по request ID и безопасной структурной evidence-ссылке; живой lease нельзя сверить. Сверка известного terminal outcome при превышении резерва сохраняет outcome/output/validation и не возобновляет job. Доверенный resumer возвращает budget-waiting job в `ready`, когда сохранённый резерв снова помещается в лимит либо начинается новый UTC-месяц; повторяющийся gateway-resumer выбирает ограниченные пакеты после восстановления provider. Финальная проверка резерва остаётся атомарной.
...
Синтетические проверки покрывают точный предел `$49.9 + $0.099999`, повторную проверку barrier между count и reserve, ожидание и автоматическое возобновление после UTC rollover и gateway outage, обработку очереди более одного пакета, три конкурентных вызова, отдельную семью, неизвестный outcome, 429/408/5xx, границы input/output usage, подсчёт только начатых generation, запрет сверки живого lease, предел decimal при maintenance-сверке, сверку terminal результата сверх резерва, least-privilege maintenance, rollback settlement вместе с job, очистку безопасных orphan attempts, provider model/usage evidence, USDC и exact HTTP shape. Обязательная suite завершается ошибкой без изолированного PostgreSQL.
