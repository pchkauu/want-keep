...
Тесты проверяют account-only page без выдуманного observation, evidence-before-commit, typed MFA failure без финансового apply, повтор и конкурентную доставку, точность PostgreSQL NUMERIC, family/job-derived actor, устойчивые account/source identities и quarantine stale result. Golden fixtures проверяют финансовый смысл, а не только JSON shape.
