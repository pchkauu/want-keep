...
Тесты проверяют account-only page без выдуманного observation, manifest mismatch до evidence, provider failure lifecycle без финансового apply, повтор и конкурентную доставку, точность PostgreSQL NUMERIC, family/job-derived actor, устойчивые account/source identities и quarantine stale result. Ingestion fixtures удаляют собственную синтетическую БД после каждого теста; повторный suite не заполняет PostgreSQL tmpfs. Golden fixtures проверяют финансовый смысл, а не только JSON shape.
