...
Suite проверяет RUB, USD, USDT, USDC, BTC и ETH с произвольной точностью; 50/50, 60/40, exact amounts, deterministic remainder и mixed receipt `1000 → A 400 + B 600`. Проверяются item override, сохранение и перерасчёт purchase fallback, fee в третьем активе, unallocated, rule precedence/conflict/preview, подтверждённый merchant alias без доверия provider ID, отсутствие ретроактивного изменения, correction/undo/source protection, полный hash review и один эффект при matching нескольких evidence.
