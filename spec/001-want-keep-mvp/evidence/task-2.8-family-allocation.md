...
Suite проверяет RUB, USD, USDT, USDC, BTC и ETH с произвольной точностью; 50/50, 60/40, exact amounts, deterministic remainder и mixed receipt `1000 → A 400 + B 600`. Проверяются item override, перерасчёт share-based и amount-based purchase fallback без превращения производных item snapshots в явные overrides, fee в третьем активе и повтор одного `MembershipID` в разных native-активах. Rule precedence/conflict/preview, сохранение `rule_conflict` в ручном и импортированном факте, подтверждённый merchant alias без доверия provider ID, отсутствие ретроактивного изменения, correction/undo/source protection, полный hash review и один эффект при matching нескольких evidence также входят в проверку.
