...
D-43 binds admission to environment, adapter/collector build, contract, allowlist, non-secret configuration and operator permission. The `backend/internal/connections/admission/` application boundary and task-1.3 storage adapter own the aggregate/repository and atomic combine/invalidate transition. task-4.x proves provider evidence and task-8.x proves host/deployment evidence; only the admission service combines both passes. A stale/missing binding yields `provider_not_admitted` before a job or collector IO. Pre-admission conformance runs in quarantine without source records or postings.
