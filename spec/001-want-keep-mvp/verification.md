...
[RU evidence](evidence/bybit.md) / [EN](evidence/bybit.en.md): D-36, BYBIT-E01–E10, точная матрица официальных read-маршрутов и [восемь синтетических сценариев](evidence/bybit.samples.json). Funding/история, P2P/eligibility, API management и Earn проверены в Google Chrome. Шесть публичных GET успешны; API-ключей нет, авторизованные запросы не выполнялись. Текущий кабинет не удовлетворяет advertiser prerequisites P2P.

BYBIT-B01 закрыт; BYBIT-B02–B05 остаются в BLK-04 под task-0.10. BYBIT-B06 и неиспользуемые продукты не блокируют. Исследование завершено по README, task-4.4 и весь MVP Not Ready. REQ-002/REQ-003/REQ-039/REQ-045, AC/task сохраняют ID, добавлен USDC; AC-071/task-6.5 больше не требуют Bybit trading/mining. AC-070 заменён в Bybit на AC-033/AC-039 без требования кредитных продуктов. Факт дохода, прогноз и отсутствующая база Earn разделены.

Локальный self review проверяет RU/EN смысл/ID, направленные движения, единицы APR/time, комиссии, replay, partial history и отсутствие личных данных. Предыдущий Avida pass не покрывает это исследование. Итоговые make docs-check, восемь Decimal-сценариев, links/diff и публикация проверяются на точном кандидате; результаты и GitHub readback фиксируются при поставке. Private API, полный backfill, действительные Earn-потоки, второй владелец/reauth/revocation, hourly collector и выбранный VPS не проверены: нет ключа/контрактов и финансового runtime. Существующая основа task-1.1 этого не доказывает.

Итоговый кандидат на базе `cce4a4b`: `make docs-check` — pass, 14 тестов генератора — pass; 87 REQ / 105 AC / 67 задач / 35 экранов / 67 GitHub mappings. Восемь синтетических Decimal/identity/coverage-сценариев, RU/EN ID/маршруты, уникальность D-ID, точный состав изменений catalog и проверка публичных артефактов — pass. Локальный self review завершён; исправлены неоднозначность формулировки P2P-доступа и формат строки словаря. Состав публикации отделён от параллельного исследования Raif.
