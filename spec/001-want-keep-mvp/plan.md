...
Entry: task-1.1 в target; contracts version 8. Exit: Money/Asset/Rate/coverage, generated OpenAPI boundary, errors `source_partial`, `source_ambiguous`, `valuation_unavailable`, `quote_unavailable`, `command_expired`, `provider_not_admitted`; connection `deploymentGate.status` и binding; sync до admission не запускает collector; D-41 recent/detail/tombstone semantics покрыты тестами.
