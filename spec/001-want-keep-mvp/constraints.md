...
task-1.1 выполнена досрочно по явному решению владельца как независимая техническая основа. Это не меняет общий verdict Not Ready. D-37 отдельно разрешает task-1.2: доменные типы и общий OpenAPI/bindings не ждут остаточного ресерча. Перед реализацией зависимых адаптеров/продуктовых правил task-0.10 закрывает соответствующие поля/источники/условия и публикует Ready-план. Неопределённые external contracts не заполняются вымышленными endpoint.
