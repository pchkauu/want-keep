...
Миграция 016 добавляет только append-only попытки и их состояния. Household lock сериализует семейный бюджет и два слота. Маркер external-started сохраняется до IO; provider IO всегда вне транзакции. Completion записывается в той же household-транзакции, что job receipt. Доверенный resumer только переводит подходящие budget-waiting jobs обратно в `ready`; резерв и слот повторно проверяются под той же блокировкой. Maintenance role может читать попытки, добавлять сверку и менять только разрешённые поля unresolved job; application role не обновляет неизменяемую историю. [Контракт и проверка](evidence/task-5.1-openai-gateway.md).
