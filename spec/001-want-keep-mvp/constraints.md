...
SDD Ready и эксплуатационный допуск разделены. Каждый provider deployment выключен по умолчанию. `backend/internal/connections/admission/` — application owner aggregate/repository interface; task-1.3 реализует storage adapter и атомарные transitions. task-4.x подтверждает provider evidence, task-8.x — host/deployment evidence; только server-owned admission service объединяет оба результата для точного D-43 binding build/contract/allowlist/config/permission/environment. Смена binding или failed/revoked check атомарно возвращает `pending|blocked`, инвалидирует не начатые jobs и проверяется collector перед provider IO. Провал gate блокирует только соответствующий connector deployment или production, а не разработку домена и других адаптеров.
