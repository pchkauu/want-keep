...
Контракт fail closed: только read capability, строгий JSON, согласованные Unicode/byte limits и точное эхо job/binding/revision/cursor. Manifest точной admitted-сборки ограничивает product, log namespace, record kind и необходимое read-действие до сохранения evidence. Типизированный provider outcome сохраняет retry/reauth/terminal lifecycle под тем же fence. Stale result пересекает только evidence/quarantine boundary. Реальные provider routes, network isolation и live permission доказываются следующими задачами; наличие synthetic gateway не является admission в production.
