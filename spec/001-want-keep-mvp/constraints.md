...
SDD Ready и эксплуатационный допуск разделены. Каждый provider deployment выключен по умолчанию. task-4.x подтверждает provider evidence, task-8.x — host/deployment evidence; только server-owned admission service объединяет оба результата для точного D-43 binding build/contract/allowlist/config/permission/environment. Смена binding или failed/revoked check атомарно возвращает `pending|blocked` до нового sync. Провал gate блокирует только соответствующий connector deployment или production, а не разработку домена и других адаптеров.
