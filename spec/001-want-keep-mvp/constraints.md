...
Контракт fail closed: только read capability, строгий JSON, каноническое UTC-время с `Z`, согласованные Unicode code-point/byte limits и точное эхо job/binding/revision/cursor. NUL и lone surrogate запрещены до identity/storage; составные ключи кодируются структурно. Manifest точной admitted-сборки связывается с provider binding и ограничивает product, log namespace, record kind и необходимое read-действие до сохранения evidence. Карточный alias допускает только безопасное название и собственные последние четыре ASCII-цифры. Типизированный provider outcome сохраняет retry/reauth/terminal lifecycle под тем же fence. Stale result пересекает только evidence/quarantine boundary. Реальные provider routes, network isolation и live permission доказываются следующими задачами; наличие synthetic gateway не является admission в production.
