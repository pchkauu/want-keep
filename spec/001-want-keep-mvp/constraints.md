...
task-1.1 выполнена досрочно по явному решению владельца как независимая техническая основа. Это не меняет общий verdict Not Ready. Прежде чем генерировать bindings, писать адаптеры или продуктовую логику, task-0.10 закрывает поля/источники/условия, обновляет эти документы и публикует Ready-план. Неопределённые external contracts не заполняются вымышленными endpoint.
