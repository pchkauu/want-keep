...
`GET/POST /transactions`, `GET /transactions/{transactionId}` and `POST /transfers` use the shared cookie/Origin/CSRF/no-store guard. Either member may record on any household account. Actor comes from the trusted session; payer, owner and author remain independent. Attachments must belong to the household, be accepted and reference the selected account. Merchant/note are retained. Only unresolved allocation and new transfers are active; category/shares and nonempty existingTransactions explicitly fail with 422 feature_unavailable instead of ignoring fields. Other product routes remain inactive.
