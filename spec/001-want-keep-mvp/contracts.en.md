...
`Account.balance` is the ledger projection with explicit quality; `opening` explains its starting input. `sourceBalance` independently reports provider amounts, credit limit, source/fetch instants, evidence and connection. Equal-asOf contradictory observations make the selected source values unknown; late older snapshots do not replace newer ones. `fundingAvailability` uses the same policy as reserves: cash projection, or exclusively a fresh complete provider observation with proven own availability, known owned/locked amounts, available ≤ owned − locked and no newer posted journal effects. Otherwise it is unknown with a reason. Debt and credit limit never increase own funds. No source/ledger addition or USD/USDT/USDC parity is performed. Full reconciliation and specialized debt/hold transitions remain task-3.2/task-2.2.
