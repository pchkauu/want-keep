...
Контракт версии 10 описывает server-issued job, capability только чтения, точный D-43 binding/admissionRevision, cursor/replay, coverage, evidence и типизированные account/balance/transaction/failure records. Provider, product/log namespace, record kind и read action проверяются по manifest до evidence; строки используют Unicode code points с запретом NUL/lone surrogate и отдельными byte limits. Время канонично в UTC с Z. Составная D-39 identity кодируется структурно; alias не пропускает PAN/CVV. Raw evidence сохраняется до финансового commit. Сервер назначает principal, external owner, internal IDs/revisions, evidence reference и fetchedAt. Каждая страница повторяет account descriptor для balance/posting; account-only page не создаёт observation. Partial omissions и admission/generation/lease/cursor fencing выполняются через CommitPage; stale result остаётся только в quarantine. Reauth/MFA/CAPTCHA переводят job в ожидание, temporary/rate-limit — в ограниченный retry, terminal failure не создаёт финансовый эффект. Generated DTO не входят в domain, суммы остаются decimal-строками, unknown/unavailable и unsupported assets не становятся нулём или паритетом.
...
Точные шесть активов, строгий JSON/evidence, provider-bound read capability, каноническое время, единые Unicode limits, структурная D-39 identity, безопасные alias, partial coverage, replay/cursor, provider failure lifecycle и stale-admission quarantine проходят без потери точности, подмены principal или повторного финансового эффекта.
...
Contract version 10 defines a server-issued job, read-only capabilities, exact D-43 binding/admissionRevision, cursor/replay, coverage, evidence and typed account/balance/transaction/failure records. Provider, product/log namespace, record kind and read action are checked against the manifest before evidence; strings use Unicode code points with NUL/lone-surrogate rejection and separate byte limits. Instants use canonical UTC Z. Composite D-39 identity is structurally encoded and aliases cannot carry PAN/CVV. Raw evidence is durable before the financial commit. The server assigns principal, external owner, internal IDs/revisions, evidence reference and fetchedAt. Every page repeats an account descriptor for each balance/posting; an account-only page creates no observation. Partial omissions and admission/generation/lease/cursor fencing use CommitPage; stale results remain only in quarantine. Reauthentication/MFA/CAPTCHA move jobs to waiting, temporary/rate-limit failures use bounded retry, and terminal failures create no financial effect. Generated DTOs never enter the domain, money remains decimal strings, and unknown/unavailable or unsupported assets never become zero or parity.
...
Six exact assets, strict JSON/evidence, provider-bound read capabilities, canonical time, aligned Unicode limits, structural D-39 identity, safe aliases, partial coverage, replay/cursor, provider-failure lifecycle and stale-admission quarantine pass without precision loss, principal spoofing or duplicate financial effects.
