...
Six exact assets, confirmed RUR → RUB with the raw code retained, strict JSON/null/evidence, exact gateway/job binding, canonical time, aligned Unicode/gap/lookback limits, structural D-39 identity, safe aliases, separate fee postings without free-form feeId, partial coverage on account/source ambiguity, provider-failure sync-result cursor fencing, staged/rejected-result disposition and same-page D-39 preflight, replay/cursor and stale-admission quarantine pass without precision loss, principal spoofing or duplicate financial effects.
