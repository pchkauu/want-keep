...
**Состояние:** Реализованы versioned wire-контракт, exact gateway binding/cursor fence, строгие Go/TypeScript boundary, accounts-owned asset normalization, durable evidence ownership/disposition и атомарное применение synthetic ingestion pages с безопасной частичной обработкой неоднозначности; реальные provider IO, UI и production остаются последующим задачам.
...
Контракт версии 10 описывает server-issued job, capability только чтения, точный D-43 binding/admissionRevision, cursor/replay, coverage, evidence и типизированные account/balance/transaction/failure records. Server-owned gateway несёт полный immutable binding и должен совпасть с job до manifest/provider IO. Provider, product/log namespace, record kind и read action проверяются по manifest до evidence. Строки используют Unicode code points с запретом NUL/lone surrogate и отдельными byte limits; gaps уникальны и ограничены 100 значениями, а maximumLookbackDays либо отсутствует, либо равен 1–36500. Время канонично в UTC с Z. Составная D-39 identity кодируется структурно; alias не пропускает PAN/CVV. Fee передаётся отдельной проводкой, а свободный feeId запрещён до типизированной correspondence. Raw evidence сохраняется с server-derived household/job до финансового commit; отклонённая после staging page получает durable rejected_result. Сервер назначает principal, external owner, internal IDs/revisions, evidence reference и fetchedAt. Каждая страница повторяет account descriptor для balance/posting; account-only page не создаёт observation. Accounts-owned policy применяет только подтверждённые mappings, включая Raiffeisen/Ozon RUR → RUB, и сохраняет raw code. Server-side account/source ambiguity сохраняет omissions, делает coverage partial, не создаёт неподтверждённый эффект и не откатывает независимые записи. Admission/generation/lease/cursor fencing выполняется через CommitPage; stale result остаётся только в quarantine. Provider failure повторяет точный issued cursor; cursor входит в attempt fence, поэтому запоздалый outcome не меняет job после продвижения checkpoint. Provider failure атомарно связывает evidence с household/job и переводит job в ожидание, ограниченный retry или terminal failure. Generated DTO не входят в domain, суммы остаются decimal-строками, unknown/unavailable и unsupported assets не становятся нулём или паритетом.
...
Точные шесть активов, подтверждённое RUR → RUB с сохранением raw code, строгий JSON/evidence, exact gateway/job binding, каноническое время, единые Unicode/gap/lookback limits, структурная D-39 identity, безопасные alias, отдельные fee postings без свободного feeId, partial coverage при account/source ambiguity, provider failure cursor fence, rejected-result disposition, replay/cursor и stale-admission quarantine проходят без потери точности, подмены principal или повторного финансового эффекта.
...
**Status:** The versioned wire contract, exact gateway binding/cursor fence, strict Go/TypeScript boundary, accounts-owned asset normalization, durable evidence ownership/disposition and atomic synthetic-page application with safe partial ambiguity handling are implemented; live provider IO, UI and production remain downstream.
...
Contract version 10 defines a server-issued job, read-only capabilities, exact D-43 binding/admissionRevision, cursor/replay, coverage, evidence and typed account/balance/transaction/failure records. The server-owned gateway carries the full immutable binding and must match the job before manifest/provider IO. Provider, product/log namespace, record kind and read action are checked against the manifest before evidence. Strings use Unicode code points with NUL/lone-surrogate rejection and separate byte limits; gaps are unique and capped at 100 values, while maximumLookbackDays is either omitted or 1–36500. Instants use canonical UTC Z. Composite D-39 identity is structurally encoded and aliases cannot carry PAN/CVV. Fees use separate postings, while free-form feeId is forbidden until typed correspondence exists. Raw evidence is durable with server-derived household/job ownership before the financial commit; a page rejected after staging receives a durable rejected_result. The server assigns principal, external owner, internal IDs/revisions, evidence reference and fetchedAt. Every page repeats an account descriptor for each balance/posting; an account-only page creates no observation. An accounts-owned policy applies only confirmed mappings, including Raiffeisen/Ozon RUR → RUB, and preserves the raw code. Server-side account/source ambiguity retains omissions, makes coverage partial, creates no unconfirmed effect and does not roll back independent records. Admission/generation/lease/cursor fencing uses CommitPage; stale results remain only in quarantine. A provider failure echoes the exact issued cursor; the cursor participates in the attempt fence, so a delayed outcome cannot change the job after checkpoint advancement. A provider failure atomically associates evidence with household/job and moves the job to waiting, bounded retry or terminal failure. Generated DTOs never enter the domain, money remains decimal strings, and unknown/unavailable or unsupported assets never become zero or parity.
...
Six exact assets, confirmed RUR → RUB with the raw code retained, strict JSON/evidence, exact gateway/job binding, canonical time, aligned Unicode/gap/lookback limits, structural D-39 identity, safe aliases, separate fee postings without free-form feeId, partial coverage on account/source ambiguity, provider-failure cursor fencing, rejected-result disposition, replay/cursor and stale-admission quarantine pass without precision loss, principal spoofing or duplicate financial effects.
