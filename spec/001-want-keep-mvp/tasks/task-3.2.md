...
**Состояние:** Реализованы versioned wire-контракт, exact gateway binding, строгие Go/TypeScript boundary, durable provider-outcome evidence и атомарное применение synthetic ingestion pages с безопасной частичной обработкой неоднозначности; реальные provider IO, UI и production остаются последующим задачам.
...
**Status:** The versioned wire contract, exact gateway binding, strict Go/TypeScript boundary, durable provider-outcome evidence and atomic synthetic-page application with safe partial ambiguity handling are implemented; live provider IO, UI and production remain downstream.
...
Six exact assets, strict JSON/evidence, exact gateway/job binding, canonical time, aligned Unicode/gap/lookback limits, structural D-39 identity, safe aliases, separate fee postings without free-form feeId, partial coverage on account/source ambiguity, provider-failure evidence, replay/cursor and stale-admission quarantine pass without precision loss, principal spoofing or duplicate financial effects.
