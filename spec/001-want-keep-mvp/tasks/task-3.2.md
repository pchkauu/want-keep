...
Контракт версии 10 описывает server-issued job, capability только чтения, точный D-43 binding/admissionRevision, cursor/replay, coverage, evidence и типизированные account/balance/transaction/failure records. Product/log namespace и record kind каждого результата проверяются по manifest до сохранения evidence; строковые maxLength одинаково измеряются Unicode code points, а byte limits применяются отдельно. Raw evidence сохраняется до финансового commit. Сервер назначает principal, external owner, internal account/operation/source revision, evidence reference и fetchedAt. Каждая самостоятельная страница повторяет account descriptor для balance/posting; account-only page не создаёт observation. D-39 dedup, partial omissions и admission/generation/lease/cursor fencing выполняются через существующий CommitPage; stale result остаётся только в quarantine. Reauth/MFA/CAPTCHA переводят job в ожидание, temporary/rate-limit — в ограниченный retry, terminal failure не создаёт финансовый эффект. Generated DTO не входят в domain, суммы остаются decimal-строками, unknown/unavailable и unsupported assets не становятся нулём или паритетом.
...
Точные шесть активов, строгий JSON/evidence, manifest-enforced read capability, единые Unicode limits, D-39 identity/revision, partial coverage, replay/cursor, provider failure lifecycle и stale-admission quarantine проходят без потери точности, подмены principal или повторного финансового эффекта.
...
Contract version 10 defines a server-issued job, read-only capabilities, exact D-43 binding/admissionRevision, cursor/replay, coverage, evidence and typed account/balance/transaction/failure records. Every result product/log namespace and record kind is checked against the manifest before evidence storage; string maxLength consistently uses Unicode code points while byte limits remain separate. Raw evidence is durable before the financial commit. The server assigns principal, external owner, internal account/operation/source revision, evidence reference and fetchedAt. Every self-contained page repeats an account descriptor for each balance/posting; an account-only page creates no observation. D-39 dedup, partial omissions and admission/generation/lease/cursor fencing use the existing CommitPage; stale results remain only in quarantine. Reauthentication/MFA/CAPTCHA move jobs to waiting, temporary/rate-limit failures use bounded retry, and terminal failures create no financial effect. Generated DTOs never enter the domain, money remains decimal strings, and unknown/unavailable or unsupported assets never become zero or parity.
...
Six exact assets, strict JSON/evidence, manifest-enforced read capabilities, aligned Unicode limits, D-39 identity/revision, partial coverage, replay/cursor, provider-failure lifecycle and stale-admission quarantine pass without precision loss, principal spoofing or duplicate financial effects.
