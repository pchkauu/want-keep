...
INSERT INTO want_keep.sync_progress(household_id,connection_id,generation,cursor,coverage,gaps,last_job_id,completed)
 SELECT DISTINCT ON(household_id,connection_id) household_id,connection_id,connection_generation,cursor,coverage,gaps,id,state='succeeded' FROM want_keep.jobs WHERE kind='sync' ORDER BY household_id,connection_id,deadline DESC,id;
