...
func TestCommandReplayVisibilityAndUnknownOutcome(t *testing.T) {
	...
	cmd, err := command.NewCommand("10000000-0000-4000-8000-000000000001", "transactions.create", hash, a)
	if err != nil {
		t.Fatal(err)
	}
	if cmd.Status() != command.Pending || cmd.CheckReplay(a, cmd.Kind(), hash) != nil {
	...
	if !errors.Is(cmd.CheckReplay(a, cmd.Kind(), strings.Repeat("b", 64)), command.ErrDuplicateCommand) {
		t.Fatal("changed payload accepted")
	}
	if !errors.Is(cmd.CheckReplay(a, "goals.create", hash), command.ErrDuplicateCommand) {
	...
	completed, err := cmd.Succeed(command.Result{ResourceType: "transaction", ResourceID: "record", Revision: 2})
	if err != nil || cmd.Status() != command.Pending || completed.Status() != command.Succeeded {
		t.Fatal("command state incorrect")
	}
	if completed.CheckReplay(a, cmd.Kind(), hash) != nil {
		t.Fatal("completed replay should return existing result")
	}
	if _, err := completed.Fail("internal_error"); !errors.Is(err, command.ErrFinalCommand) {
		t.Fatal("final command changed")
	}
	if _, err := cmd.Fail("provider secret details"); err == nil {
		t.Fatal("unsafe failure accepted")
	}
	failed, err := cmd.Fail("version_conflict")
	if err != nil || failed.Status() != command.Failed || failed.ErrorCode() != "version_conflict" {
		t.Fatal("failed status lost")
	}
	if _, err := command.NewCommand("bad", cmd.Kind(), hash, a); err == nil {
		t.Fatal("invalid ID accepted")
	}
	if _, err := command.NewCommand(cmd.ID(), cmd.Kind(), hash, household.Principal{}); err == nil {
