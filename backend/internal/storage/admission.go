...
func (s *Store) Connection(ctx context.Context, p household.Principal, id string) (admission.Connection, error) {
	...
	err = q.QueryRow(ctx, `SELECT household_id,provider,external_owner_id,generation,authorized FROM want_keep.connections WHERE household_id=$1 AND id=$2`, p.HouseholdID(), id).Scan(&c.HouseholdID, &c.Provider, &c.Owner, &c.Generation, &c.Authorized)
	if errors.Is(err, pgx.ErrNoRows) {
