...
func (s *Store) CreateReplayJob(ctx context.Context, c admission.Connection, a connections.Admission, deadline time.Time, requestID string, from, to time.Time) (jobs.Job, error) {
	...
	existing, err := scanReplayJob(scope.tx.QueryRow(ctx, `SELECT `+replayJobColumns+` FROM want_keep.jobs WHERE household_id=$1 AND replay_request_id=$2`, scope.principal.HouseholdID(), requestID))
	if err == nil {
