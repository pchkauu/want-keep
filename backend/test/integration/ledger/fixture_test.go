...
func TestMain(m *testing.M) {
	...
	if err != nil || raw == "" || u.Path != "/want_keep_test" || (u.Hostname() != "localhost" && (net.ParseIP(u.Hostname()) == nil || !net.ParseIP(u.Hostname()).IsLoopback())) {
		fmt.Fprintln(os.Stderr, "Ledger integration requires an isolated loopback PostgreSQL database named want_keep_test via WANT_KEEP_TEST_DATABASE_URL.")
		os.Exit(2)
	}
	databaseURL = u
...
func newDatabase(t *testing.T) (*pgxpool.Pool, string) {
	t.Helper()
	name := "wk_ledger_" + strings.ReplaceAll(uuid.NewString(), "-", "")
