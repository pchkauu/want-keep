...
func (f *fixture) client(p household.Principal) *client {
	...
	lh, err := ledgerdelivery.New(f.ledgerService(), f.matching(), journal.NewQueries(f.store), f.executor, queries, sessions, f.store, security.Config{Environment: "test", Origin: "http://localhost"}, func() calendar.Instant { return f.now })
	if err != nil {
		f.t.Fatal(err)
	}
