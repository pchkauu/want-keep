...
func TestMain(m *testing.M) {
	...
	if err != nil || raw == "" || u.Path != "/want_keep_test" || (u.Hostname() != "localhost" && (net.ParseIP(u.Hostname()) == nil || !net.ParseIP(u.Hostname()).IsLoopback())) {
		fmt.Fprintln(os.Stderr, "Matching integration requires an isolated loopback PostgreSQL database named want_keep_test via WANT_KEEP_TEST_DATABASE_URL.")
		os.Exit(2)
	}
	databaseURL = u
...
func newDatabase(t *testing.T) (*pgxpool.Pool, string) {
	t.Helper()
	name := "wk_matching_" + strings.ReplaceAll(uuid.NewString(), "-", "")
...
func newFixture(t *testing.T) *fixture { return newFixtureUsingMigrations(t, migrations.Files) }
func newFixtureUsingMigrations(t *testing.T, files fs.FS) *fixture {
	...
	f.writer = matching.NewService(s, journal.NewWriter(s, s), func() calendar.Instant { return f.now }, uuid.NewString)
...
func (f *fixture) restart() {
	...
	f.store = store
	f.writer = matching.NewService(store, journal.NewWriter(store, store), func() calendar.Instant { return f.now }, uuid.NewString)
